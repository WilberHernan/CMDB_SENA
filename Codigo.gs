...
function obtenerValidacionesManuales() {
  ...
  // Merge con validaciones por defecto: las dinámicas de Hoja3 prevalecen,
  // pero si algún campo no está en Hoja3, usamos el fallback por defecto.
  const porDefecto = obtenerValidacionesPorDefecto();
  Object.keys(porDefecto).forEach(function(idx) {
    const i = parseInt(idx);
    if (!resultado[i]) {
      resultado[i] = porDefecto[i];
    }
  });
...
function obtenerValidacionesPorDefecto() {
  ...
  resultado[51] = resultado[49]; // RESPONSABLE DEL PRIMER MANTENIMIENTO ABRIL 2026
  resultado[52] = resultado[49]; // RESPONSABLE DEL SEGUNDO MANTENIMIENTO OCTUBRE 2026
  return resultado;
}
